// i18n.js — UI strings. English is the default; zh-TW (Traditional Chinese)
// is selectable via the masthead toggle and persisted in localStorage.
// Song titles, composer names, chord symbols, and style names stay as-is.

export const STRINGS = {
  en: {
    "mode.session": "Session",
    "mode.inspire": "Inspire",
    "mode.live": "Live",
    "live.k": "the room",
    "live.listen": "start listening",
    "live.stop": "stop listening",
    "live.record": "record a take",
    "live.recording": "stop & keep",
    "live.you": "you",
    "live.heat": "heat",
    "live.take": "takes",
    "live.download": "download",
    "live.delete": "delete",
    "live.headphones": "headphones keep the band out of the mic — on speakers it hears itself a little",
    "live.idle": "the band plays its own arc until you start",
    "live.listening": "the band takes what it hears into the next chorus",
    "live.denied": "no microphone — the browser said no, and the band will play its own arc",
    "live.armed": "recording from the count-in",
    "live.responseK": "answers",
    "live.respBar": "next bar",
    "live.respPhrase": "next phrase",
    "live.respChorus": "next chorus",
    "live.responseSub": "how soon it follows",
    "live.takeover": "you laid out — the band has it for a phrase",
    "live.settled": "the band is sitting on it, waiting for you",
    "live.waiting": "the mic waits for the band to start",
    "live.beta": "beta",
    tunes: "tunes",
    addTune: "+ add a tune",
    sleeveCount: "{n} standards",
    jumpTo: "jump to",
    allTunes: "all tunes",
    searchPh: "search tunes…",
    noMatch: "no tunes match",
    sleeveNote: "export JSON and open a PR to share it",
    soloNotes: "solo notes · ",
    played: "played",
    soloistStyle: "soloist style · ",
    newTake: "new take",
    takeLabel: "take",
    holdTake: "hold this line",
    rampOff: "off",
    chromaticOpt: "chromatic notes",
    rig: "the rig",
    rampK: "ramp",
    rampSub: "each chorus",
    countK: "choruses",
    countSub: "then stop",
    chorusK: "chorus",
    readingSub: "what you read",
    keyChordTone: "chord tone",
    keyTension: "colour",
    keyApproach: "chromatic",
    keyOutside: "outside",
    keyOnBeat: "on the beat",
    keyHeld: "held",
    lineWait: "the line appears when the band starts",
    lineLoading: "loading the soloist…",
    takeTip:
      'Every solo is improvised from this number. The same one always plays the same line, so write down a take you liked and type it back to hear it again. "New take" rolls a fresh one.',
    crowding: "crowding",
    sparse: "sparse",
    crowded: "crowded",
    phrasing: "phrasing",
    short: "short",
    long: "long",
    voicing: "voicing",
    mono: "mono",
    multi: "multi",
    soloistK: "the soloist",
    styleK: "style",
    soloInstSub: "the voice",
    crowdSub: "how many notes",
    phraseSub: "how long a line",
    voicingSub: "one note or more",
    chromaticK: "chromatic",
    chromaticSub: "notes outside",
    stepSparse: "sparse",
    stepLight: "light",
    stepEven: "even",
    stepBusy: "busy",
    stepCrowded: "crowded",
    stepShortest: "clipped",
    stepMedium: "medium",
    stepLongest: "spun out",
    soloInst: "played on",
    instPiano: "piano",
    instVibes: "vibes",
    next: "next · {chord}",
    bars: "bars {from}–{to} of {total}",
    creditsHead: "sources & credits",
    reportProblem: "report a problem →",
    tipWhy: "free, open source, and staying that way.",
    tipJar: "tip jar",
    tipOpenPage: "open the page instead →",
    credits1:
      'This tool displays <strong>chord symbols only</strong> — no melodies, no lyrics, no recordings. Song titles and composer names are shown as factual metadata. Chord changes follow common lead-sheet practice, cross-checked against jazz-education references.',
    verified: "changes for this song verified against: ",
    creditsWjd:
      'The soloist\'s phrasing figures are measured from the <a href="https://jazzomat.hfm-weimar.de/" rel="noopener">Weimar Jazz Database</a> — 456 hand-transcribed solos from the Jazzomat Research Project, Hochschule fur Musik Franz Liszt Weimar, under the <a href="https://opendatacommons.org/licenses/odbl/" rel="noopener">Open Database License (ODbL 1.0)</a>. No transcription is reproduced; what ships is derived statistics.',
    credits2:
      'Samples: <a href="https://github.com/gleitz/midi-js-soundfonts" rel="noopener">MusyngKite soundfont kit (CC BY-SA 3.0)</a>, the Splendid Grand Piano, and the vibraphone from the <a href="https://github.com/sgossner/VCSL" rel="noopener">Versilian Community Sample Library (CC0)</a>, via <a href="https://github.com/danigb/smplr" rel="noopener">smplr (MIT)</a> · scheduling by <a href="https://tonejs.github.io/" rel="noopener">Tone.js (MIT)</a> · drums synthesized in-browser. "Real" pack (all CC0): <a href="https://github.com/sfzinstruments/karoryfer.meatbass" rel="noopener">Meatbass</a>, <a href="https://github.com/sfzinstruments/karoryfer.pastabass" rel="noopener">Pastabass</a>, <a href="https://github.com/sfzinstruments/karoryfer.black-and-green-guitars" rel="noopener">Black &amp; Green Guitars</a> and <a href="https://github.com/sfzinstruments/karoryfer.swirly-drums" rel="noopener">Swirly Drums</a> by Karoryfer Samples · <a href="https://github.com/sfzinstruments/virtuosity_drums" rel="noopener">Virtuosity Drums</a> by Versilian Studios. This site is <a href="https://github.com/HaKexva/the-woodshed" rel="noopener">open source under GPLv3</a>. For personal practice &amp; education.',
    tempo: "tempo",
    band: "band",
    play: "play",
    "dock.more": "more controls",
    pause: "pause",
    resume: "resume",
    stop: "stop",
    "mute.keys": "keys",
    "mute.gtr": "gtr",
    "mute.bass": "bass",
    "mute.drums": "drums",
    boost: "Bass+",
    boostSub: "speakers only",
    boostOn: "on",
    reading: "reading",
    readC: "C",
    comp: "comp",
    compPlain: "plain",
    compWarm: "warm",
    compHot: "hot",
    compSub: "how full",
    loopK: "loop",
    loopSub: "one passage",
    keyK: "key",
    keySub: "what sounds",
    keyAsWritten: "as written",
    keyPlayIn: "play it in",
    keyRound: "round the keys",
    keyStep1: "+1 a chorus",
    keyStep5: "+5 a chorus",
    keyStep7: "+7 a chorus",
    feel: "feel",
    feelAuto: "as written",
    feelBallad: "ballad",
    feelModal: "modal",
    feelSwing: "swing",
    feelBlues: "blues",
    feelBossa: "bossa",
    feelLatin: "latin",
    feelFunk: "funk",
    feelSub: "how it is played",
    hint: "play / stop",
    "status.loading": "loading instruments… {n}/{total}",
    "status.loadingSolo": "loading the soloist…",
    "status.stalled": "the audio stopped — tap anywhere to carry on",
    "status.audioGone": "the audio device went away — reload to carry on",
    "status.loadingHq": "loading Real sounds… {n}/{total} ·",
    hqSkip: "use standard",
    "ed.heading": "add a tune",
    mineTunes: "mine",
    newTune: "new",
    "ed.save": "save & play",
    "ed.update": "update & play",
    "ed.delete": "delete",
    "ed.titlePh": "untitled",
    "ed.sheetEmpty": "the chart appears as you type",
    "ed.shape": "{bars} bars · {ts}",
    "ed.reads": "reads",
    "ed.exportAll": "download my tunes",
    "ed.importFile": "import a file…",
    "ed.saved": "saved as “{title}” — find it under ★ mine",
    "ed.deleted": "deleted",
    "ed.imported": "imported {n} tune(s)",
    "ed.note":
      'Saved in this browser and listed under <strong>★ mine</strong>. Only a title-less tune gets named for you — everything else has a sensible default, so changes alone are enough to hear it.',
    "err.noTunes": "no tunes in that file",
    "ed.title": "title",
    "ed.key": "key",
    "ed.bpm": "bpm",
    "ed.style": "style",
    "ed.beatsPerBar": "beats",
    "ed.changesHint":
      'changes — bars split by <code>|</code>, chords in a bar by spaces, optional beats with <code>:</code> (e.g. <code>Dm7b5:3 G7:1</code>)',
    "ed.random": "random changes",
    "ed.close": "close",
    "err.titleRequired": "title is required",
    "err.noBars": "no bars found — separate bars with |",
    "err.badChord": 'bar {n}: "{sym}" doesn\'t look like a chord',
    "err.badBeats": 'bar {n}: bad beat count in "{tok}" (whole or half beats only)',
    "err.sumMismatch": "bar {n}: beats sum to {sum}, need {ts} — use chord:beats for uneven splits",
    editPreview: "edit",
    deleteTune: "delete",
    deleteSure: "tap again",
    "blurb.parker": "relentless bebop 8ths, enclosures, barline-crossing phrases",
    "blurb.monk": "angular leaps, weak-beat jabs, sudden silences",
    "blurb.singer": "inside an octave, stepwise, and a leap comes straight back",
    "blurb.silver": "short funky riffs, repeated and squeezed, gospel smears",
  },
  zh: {
    "mode.session": "合奏",
    "mode.inspire": "靈感",
    "mode.live": "現場",
    "live.k": "現場",
    "live.listen": "開始聆聽",
    "live.stop": "停止聆聽",
    "live.record": "錄一段",
    "live.recording": "停止並保留",
    "live.you": "你",
    "live.heat": "熱度",
    "live.take": "上一段",
    "live.download": "下載",
    "live.delete": "刪掉",
    "live.headphones": "戴耳機樂團就不會被麥克風聽見 — 用喇叭時它會聽到一點自己",
    "live.idle": "還沒開始聆聽，樂團先照自己的起伏走",
    "live.listening": "樂團會把聽到的帶進下一輪",
    "live.denied": "沒有麥克風 — 瀏覽器拒絕了，樂團會照自己的起伏走",
    "live.armed": "從預備拍開始錄音",
    "live.responseK": "跟隨",
    "live.respBar": "下一小節",
    "live.respPhrase": "下一樂句",
    "live.respChorus": "下一輪",
    "live.responseSub": "多快跟上你",
    "live.takeover": "你停了 — 樂團接手一個樂句",
    "live.settled": "樂團撐著等你回來",
    "live.waiting": "麥克風等樂團開始",
    "live.beta": "測試版",
    tunes: "曲目",
    addTune: "＋新增樂曲",
    sleeveCount: "{n} 首經典曲目",
    jumpTo: "跳至",
    allTunes: "全部曲目",
    searchPh: "搜尋樂曲…",
    noMatch: "沒有符合的樂曲",
    sleeveNote: "匯出 JSON 並發 PR 分享你的樂曲",
    soloNotes: "即興音階 · ",
    played: "已奏",
    soloistStyle: "獨奏風格 · ",
    newTake: "再來一段",
    takeLabel: "編號",
    holdTake: "固定這段樂句",
    rampOff: "關閉",
    chromaticOpt: "半音經過音",
    rig: "練習台",
    rampK: "遞增",
    rampSub: "每次反覆",
    countK: "反覆",
    countSub: "次後停止",
    chorusK: "反覆",
    readingSub: "調",
    keyChordTone: "和弦音",
    keyTension: "色彩音",
    keyApproach: "半音經過",
    keyOutside: "圈外音",
    keyOnBeat: "落在正拍",
    keyHeld: "長音",
    lineWait: "樂團開始後即顯示樂句",
    lineLoading: "載入獨奏音色中…",
    takeTip: "每段獨奏都由這組編號即興生成。同一組編號永遠奏出同一段樂句，記下喜歡的編號、再輸入回來就能重聽。按「再來一段」會換一組新的。",
    crowding: "音符密度",
    sparse: "疏",
    crowded: "密",
    phrasing: "樂句長度",
    short: "短",
    long: "長",
    voicing: "聲部",
    mono: "單音",
    multi: "和音",
    soloistK: "獨奏者",
    styleK: "風格",
    soloInstSub: "音色",
    crowdSub: "音符多寡",
    phraseSub: "樂句長短",
    voicingSub: "單音或和音",
    chromaticK: "半音",
    chromaticSub: "圈外音",
    stepSparse: "很疏",
    stepLight: "偏疏",
    stepEven: "適中",
    stepBusy: "偏密",
    stepCrowded: "很密",
    stepShortest: "極短",
    stepMedium: "適中",
    stepLongest: "綿長",
    soloInst: "音色",
    instPiano: "鋼琴",
    instVibes: "鐵琴",
    next: "下一個 · {chord}",
    bars: "第 {from}–{to} 小節・共 {total} 小節",
    creditsHead: "來源與致謝",
    reportProblem: "回報問題 →",
    tipWhy: "免費、開源，而且會一直是。",
    tipJar: "小費罐",
    tipOpenPage: "改為開啟頁面 →",
    credits1:
      "本工具<strong>僅顯示和弦記號</strong>——不含旋律、歌詞或錄音。曲名與作曲者僅作為事實性資訊呈現。和弦進行依照常見的 lead sheet 慣例，並與爵士教育資源相互查證。",
    verified: "本曲和弦已比對下列來源：",
    creditsWjd:
      '獨奏的樂句素材取自 <a href="https://jazzomat.hfm-weimar.de/" rel="noopener">Weimar Jazz Database</a> 的統計——威瑪李斯特音樂院 Jazzomat 研究計畫的 456 份人工採譜，授權為 <a href="https://opendatacommons.org/licenses/odbl/" rel="noopener">Open Database License (ODbL 1.0)</a>。本站不重現任何採譜，只使用由其推導出的統計數據。',
    credits2:
      '取樣音色：<a href="https://github.com/gleitz/midi-js-soundfonts" rel="noopener">MusyngKite soundfont（CC BY-SA 3.0）</a>與 Splendid Grand Piano，經由 <a href="https://github.com/danigb/smplr" rel="noopener">smplr（MIT）</a>播放 · 排程由 <a href="https://tonejs.github.io/" rel="noopener">Tone.js（MIT）</a>驅動 · 鼓組為瀏覽器內合成。真實音色包（皆為 CC0）：Karoryfer Samples 的 <a href="https://github.com/sfzinstruments/karoryfer.meatbass" rel="noopener">Meatbass</a>、<a href="https://github.com/sfzinstruments/karoryfer.pastabass" rel="noopener">Pastabass</a>、<a href="https://github.com/sfzinstruments/karoryfer.black-and-green-guitars" rel="noopener">Black &amp; Green Guitars</a> 與 <a href="https://github.com/sfzinstruments/karoryfer.swirly-drums" rel="noopener">Swirly Drums</a> · Versilian Studios 的 <a href="https://github.com/sfzinstruments/virtuosity_drums" rel="noopener">Virtuosity Drums</a>。本站以 <a href="https://github.com/HaKexva/the-woodshed" rel="noopener">GPLv3 開源</a>。僅供個人練習與教育用途。',
    tempo: "速度",
    band: "樂團",
    play: "播放",
    "dock.more": "更多控制",
    pause: "暫停",
    resume: "繼續",
    stop: "停止",
    "mute.keys": "鋼琴",
    "mute.gtr": "吉他",
    "mute.bass": "貝斯",
    "mute.drums": "鼓",
    boost: "貝斯加強",
    boostSub: "喇叭專用",
    boostOn: "開",
    reading: "移調",
    readC: "C",
    comp: "伴奏",
    compPlain: "簡潔",
    compWarm: "適中",
    compHot: "熱情",
    compSub: "伴奏濃度",
    loopK: "循環",
    loopSub: "單段反覆",
    keyK: "移調",
    keySub: "實際音高",
    keyAsWritten: "原調",
    keyPlayIn: "移到",
    keyRound: "輪轉各調",
    keyStep1: "每次 +1",
    keyStep5: "每次 +5",
    keyStep7: "每次 +7",
    feel: "曲風",
    feelAuto: "照原譜",
    feelBallad: "抒情",
    feelModal: "調式",
    feelSwing: "搖擺",
    feelBlues: "藍調",
    feelBossa: "巴薩諾瓦",
    feelLatin: "拉丁",
    feelFunk: "放克",
    feelSub: "怎麼演奏",
    hint: "播放／停止",
    "status.loading": "載入樂器中… {n}/{total}",
    "status.loadingSolo": "載入獨奏鋼琴中…",
    "status.stalled": "音訊被中斷了 — 點一下畫面繼續",
    "status.audioGone": "音訊裝置消失了 — 重新整理才能繼續",
    "status.loadingHq": "載入真實音色中… {n}/{total} ·",
    hqSkip: "先用標準音色",
    "ed.heading": "新增樂曲",
    mineTunes: "我的",
    newTune: "新增",
    "ed.save": "儲存並播放",
    "ed.update": "更新並播放",
    "ed.delete": "刪除",
    "ed.titlePh": "未命名",
    "ed.sheetEmpty": "輸入小節後會顯示樂譜",
    "ed.shape": "{bars} 小節 · {ts}",
    "ed.reads": "可讀",
    "ed.exportAll": "下載我的樂曲",
    "ed.importFile": "匯入檔案…",
    "ed.saved": "已儲存為「{title}」——可在 ★ 我的 中找到",
    "ed.deleted": "已刪除",
    "ed.imported": "已匯入 {n} 首",
    "ed.note": "儲存在這個瀏覽器，列在 <strong>★ 我的</strong> 之下。只有未命名的樂曲會自動取名，其餘欄位都有預設值，只要輸入和弦就能聽。",
    "err.noTunes": "這個檔案裡沒有樂曲",
    "ed.title": "曲名",
    "ed.key": "調性",
    "ed.bpm": "速度",
    "ed.style": "風格",
    "ed.beatsPerBar": "拍數",
    "ed.changesHint":
      "和弦進行——小節以 <code>|</code> 分隔、同小節和弦以空格分隔，拍數可用 <code>:</code> 指定（例如 <code>Dm7b5:3 G7:1</code>）",
    "ed.random": "隨機和弦",
    "ed.close": "關閉",
    "err.titleRequired": "必須填寫曲名",
    "err.noBars": "找不到小節——請用 | 分隔小節",
    "err.badChord": "第 {n} 小節：「{sym}」看起來不是和弦",
    "err.badBeats": "第 {n} 小節：「{tok}」的拍數無效（僅限整數或半拍）",
    "err.sumMismatch": "第 {n} 小節：拍數總和為 {sum}，應為 {ts}——不均分請用 和弦:拍數",
    editPreview: "編輯",
    deleteTune: "刪除",
    deleteSure: "再按一次",
    "blurb.parker": "連綿的 bebop 八分音符、環繞音、跨小節樂句",
    "blurb.monk": "稜角跳進、弱拍重擊、驟然靜默",
    "blurb.singer": "音域不出一個八度、以級進為主，跳進後立刻折返",
    "blurb.silver": "短小放克樂句反覆推進、福音式滑音",
  },
};

let lang = localStorage.getItem("woodshed-lang") === "zh" ? "zh" : "en";

export function getLang() {
  return lang;
}

export function setLang(l) {
  lang = l === "zh" ? "zh" : "en";
  localStorage.setItem("woodshed-lang", lang);
  applyStatic();
}

/** Translate a key, with {var} interpolation. Falls back to English. */
export function t(key, vars = {}) {
  let s = STRINGS[lang][key] ?? STRINGS.en[key] ?? key;
  for (const [k, v] of Object.entries(vars)) s = s.replaceAll(`{${k}}`, v);
  return s;
}

/** Is there a string under this key at all? */
const known = (key) => key in STRINGS.en || key in STRINGS[lang];

/**
 * Re-render every statically tagged node for the current language.
 *
 * A key with no string is left alone rather than written out raw. The page and
 * the scripts are separate downloads with separate cache lifetimes, so a
 * browser will sometimes hold an old index.html against a new i18n.js — and
 * printing "ed.importLabel" where a sentence belongs turns a stale cache into
 * something that looks broken. The markup already carries readable English as
 * its own content; when the key is gone, that is the better answer.
 */
export function applyStatic() {
  document.documentElement.lang = lang === "zh" ? "zh-Hant" : "en";
  const set = (attr, apply) =>
    document.querySelectorAll(`[${attr}]`).forEach((el) => {
      const key = el.getAttribute(attr);
      if (known(key)) apply(el, t(key));
    });
  set("data-i18n", (el, v) => (el.textContent = v));
  set("data-i18n-html", (el, v) => (el.innerHTML = v));
  set("data-i18n-ph", (el, v) => (el.placeholder = v));
  // a control whose whole label is an icon still has to say what it does
  set("data-i18n-aria", (el, v) => el.setAttribute("aria-label", v));
  // an <optgroup> heading is a label attribute, not text content
  set("data-i18n-label", (el, v) => el.setAttribute("label", v));
}
