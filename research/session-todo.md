...
| 23 | `[B]` | ~~**Seed the band, not just the solo**~~ — **dropped.** The description was stale twice over: `role` and `bwave` went with item 12, and no generator calls bare `Math.random()` — they all go through `rand()`, which only falls back when no seed is active. Item 24 also collapsed the work to wrapping one call, since every generator now runs inside `_planChorus`. Dropped anyway, which means every measurement here stays an average over takes rather than an A/B | XS | **dropped** |
