...
| 2 | `[B]` | **Voice-led, non-deterministic piano voicings.** Candidate set per chord, scored by voice motion from the previous chord and by top-note recency, picked weighted-random among the best few. Fixed the frozen top line, the 84 two-note voicings and the 12 ♯9 semitone clusters in one change — 0.19 → 0.35 distinct top notes per chord, top-voice motion *down* from 2.48 to 1.57 semitones, clusters and dyads to zero. See the applied section of [backing-band-audit.md](backing-band-audit.md#1-voice-led-non-deterministic-piano-voicings--applied) | M | **done** |
...
| 8 | `[B]` | **Two-feel** — per-chorus bass feel `two \| four \| pedal \| broken`, drums following. The bass currently walks quarters from bar 1 forever. Largest missing style variation; applies to 356 of 428 tunes | S | |
...
| 18 | `[B]`/`[P]` | **Decouple comping feel from tune style** — a session-mode feel picker (swing / two-feel / bossa / latin / shuffle / even-8ths / ballad). 82.5% of tunes are tagged `swing`, so the blues branch serves 1 tune and modal serves 2. Fixes the dead branches *and* is a practice feature | M | |
